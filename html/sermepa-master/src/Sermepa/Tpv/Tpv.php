...
class Tpv
{
    ...
    {
        if ($sumTotal < 0) {
            throw new \Exception('Sum total must be greater than or equal to 0.');
        }
    ...
    public function setDateFrecuency($dateFrecuency)
    {
        if ( !is_numeric($dateFrecuency) || (strlen($dateFrecuency) < 1 || strlen($dateFrecuency) > 5) ) {
            throw new \Exception('Date frecuency is not valid.');
        }
    ...
    public function setVersion($version = '')
    {
        if ($this->isEmpty($version)) {
            throw new \Exception('Please add version.');
        }
    ...
    public static function getJsPath($environment = 'test', $version = '2'){
        ...
        if (!isset($jsPaths[$environment][$version])) {
            throw new \Exception('Invalid environment or version');
        }
    ...
    public function setMethod($method = 'C')
    {
        ...
        if (!in_array($method, ['T', 'C', 'R', 'D', 'z', 'p', 'N', 'xpay'])) {
            throw new \Exception('Pay method is not valid');
        }
    ...
    public function setParameters($parameters=[])
    {
        ...
        if(array_keys($keys) === $keys ) {
            throw new \Exception('Parameters is not an array associative');
        }
    ...
    public function setMerchantCofIni($value)
    {
        ...
        if (!in_array($value, $validOptions, true)) {
            throw new \Exception('Set Merchant COF INI valid options');
        }
    ...
    public function setMerchantCofType($value)
    {
        ...
        $value = strtoupper($value);
        if (!in_array($value, $validOptions, true)) {
            throw new \Exception('Set Merchant COF type');
        }
    ...
    function createOrderNumber($length = 12)
    {
        ...
        if (!is_int($length)) {
            throw new \Exception("The order number length must be an integer.");
        }
        
        // Verify that the length is between 4 and 12
        if ($length < 4 || $length > 12) {
            throw new \Exception("The order number length must be between 4 and 12.");
        }
